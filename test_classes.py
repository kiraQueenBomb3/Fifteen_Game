...
def win_screen(screen, steps, width, height):
    ...
    for line in outro_text:
        congratulates = font.render(line, 1, pygame.Color('black'))
        rect_for_steps = congratulates.get_rect()
        rect_for_steps.x = 10
        rect_for_steps.top = text_coord
        text_coord += 50
        screen.blit(congratulates, rect_for_steps)
    ...
    for event in pygame.event.get():
        ...
        # if event.type == pygame.KEYDOWN and pygame.key == pygame.
    pygame.display.flip()
...
def gameplay(screen, quarter, width, height, board, clock):
    ...
    while running:
        ...
        screen.fill((0, 0, 0))
        font = pygame.font.Font(None, 30)
        counting_steps = font.render(f'Total steps: {steps}', 1, pygame.Color('white'))
